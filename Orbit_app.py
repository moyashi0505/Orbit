...
def update_user_name(data: UserNameUpdate, db: Session = Depends(get_db)):
    target = db.query(UserDB).filter(UserDB.username == data.username).first()
    if target:
        target.username = data.new_name
        db.commit()
        return {"message": "Success"}
